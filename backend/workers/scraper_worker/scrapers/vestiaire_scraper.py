...
class VestiaireScraper:
    """Scraper for Vestiaire Collective website."""

    def __init__(self, brand, query, output_dir, query_hash, local=False):
        ...
        self.local = local
        self.search_url = "https://www.vestiairecollective.com/search"
        self.source = "VESTIAIRE"
    ...
    def run(self):
        ...
        utils = ScraperUtils(s3_client=s3, dynamodb_client=dynamodb)

        if self.local:
            # For local testing with saved HTML
            url = f"file:///{os.path.abspath('local_websites/vestiaire_test.html')}"
        else:
...
if __name__ == "__main__":
    ...
    scraper = VestiaireScraper(brand, query, output_dir, query_hash)
    scraper.run() 