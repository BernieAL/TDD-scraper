...
def log_step(step_name: str, details: Dict = None):
    """Helper function to log scraper steps with consistent formatting"""
    logger.info("-" * 50)
    logger.info(f"ITALIST STEP: {step_name}")
    if details:
...
class ItalistScraper(BaseScraper):
    ...
    def __init__(self, brand, query,output_dir,query_hash,local):
        ...
        log_step("Initializing Italist Scraper", {
            "brand": brand,
            "query": query,
    ...
    def get_listings(self, driver):
        ...
        try:
            log_step("Finding Product Listings")
            listings = driver.find_elements(By.XPATH, "//div[contains(@class, 'product-grid-container')]//a")
            log_step("Found Product Listings", {"count": len(listings)})
            return listings
        except NoSuchElementException as e:
            logger.error("Could not find product listings", exc_info=True)
            return []

    def extract_listing_data(self, listing):
        """Extracts the brand, product name, and price from a single listing."""
        try:
            log_step("Extracting Listing Data")
            ...
            log_step("Extracted Product Data", {
            ...
            logger.error("Error extracting listing data", exc_info=True)
